-- =============================================================================
-- 一键全量：有余额流失用户征信特征 + 标签（精确 5401 cohort）
-- =============================================================================
-- 【用法】整文件一次提交执行（已含 Part 0 drop，无需先跑 drop_all）
--
-- 【前置只读表 - 必须已存在】
--   yye_pril_bal_info_20260623_2、yye_pril_bal_info_20260623_4（同事样本）
--   lj_iceberg.pboccr2d.* 征信 9 张
--
-- 【产出】jcr_credit_feature_label_20260623（cohort_eligible=1 应为 5401 行）
-- 分步调试：yye_credit_feature_process.sql
-- =============================================================================
...
-- ########## Part 2：权威 cohort 5401（同事 yye _4）##########
drop table if exists lj_iceberg.ai_decision_dev.jcr_cohort_5401_20260623;
...
-- ########## Part 10：跑完核验 ##########
select count(1) as cohort_uuid_cnt from lj_iceberg.ai_decision_dev.jcr_cohort_5401_20260623;
select count(1) as feature_label_cnt from lj_iceberg.ai_decision_dev.jcr_credit_feature_label_20260623 where cohort_eligible = 1;
select label, count(1) as num from lj_iceberg.ai_decision_dev.jcr_credit_feature_label_20260623 where cohort_eligible = 1 and label is not null group by label order by label;
