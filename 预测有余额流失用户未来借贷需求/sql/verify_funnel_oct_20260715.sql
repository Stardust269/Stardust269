...
-- ② 最低余额日(days_dt)起 0~60 天：非5103全渠道无余额 + 5103无余额 + 未提现
--    pf 表已含 no_balance_flg_60=1 AND no_balance_flg_60_5103=1 AND with_0_30+with_31_60=0
...
-- ② 拆因：仅非5103无余额 vs 加5103无余额
select
    count(1) as s2_non5103_nb_only,
    sum(if(no_balance_flg_60_5103 = 1, 1, 0)) as s2_also_5103_nb
from lj_iceberg.ai_decision_dev.jcr_pril_bal_info_nb_20260715
where m = '202510' and no_balance_flg_60 = 1;

select
    count(1) as s2_nb5103_only,
    sum(if(no_balance_flg_60 = 1, 1, 0)) as s2_also_non5103_nb
from lj_iceberg.ai_decision_dev.jcr_pril_bal_info_nb_20260715
