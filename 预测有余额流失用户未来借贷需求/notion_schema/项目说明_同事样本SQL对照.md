...
## 六、5186 vs 5401（差 215 的原因）

| 计数方式 | 表 | 结果 |
|----------|-----|------|
| 我方自建 `_2` 同条件 WHERE | `jcr_pril_bal_info` | **5186** |
| 同事最后一查 `group by uuid` | `yye_pril_bal_info_20260623_4` | **5401** |

**原因**：
1. 同事 **5401 是在 `_4` 上数出来的**，不是 `_2` 直接 WHERE。
2. 自建 Step 0 与同事 `yye _2` 在 `no_balance`/`with`/`had` 等字段上存在约 **215 人** 的差异（引擎、跑数时点、中间表版本等）。
3. 要 **精确 5401**，必须用 Step 0d 从同事 `yye _4` 导入 uuid 名单，Step 5~7 `inner join jcr_cohort_5401_20260623`。

---

## 七、建议下一步

1. `step0c_copy_from_yye.sql` + `step0d_cohort_5401_from_yye.sql`  
2. 核验 `jcr_cohort_5401_20260623` = **5401**  
3. drop B1 → 重跑 Step 5~7  
4. `cohort_eligible=1` 必须 = **5401**
