...
- **征信 Part 2**（`fxj_seed_credit_account_base`）exit **137** 且**不能调 executor 内存**：用 `bash scripts/run_part2_account_batches.sh`（每批独立 Spark，默认 64 份）；轻量 SQL 见 `sql/fxj_seed_credit_account_part2_light.sql`（`bill_day=null`，不做 1m 开窗）。仍失败则加大 `NUM_BATCHES=128`。
